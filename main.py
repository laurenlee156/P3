...
class NetworkHandler:
                    ...
                    if distance < (int(self.ap_lst[i][11]) + int(self.ap_lst[j][11])):
                        ...
                        if self.ap_lst[i][4] == self.ap_lst[j][4]:
                            ...
                            if len(pref_channels) > 0:
                                ...
                                self.ac_log.append(f"Step {n}: AC REQUIRES {self.ap_lst[i][1]} TO CHANGE CHANNEL TO "
                                              f"{max(pref_channels)}")
    ...
    def single_ap(self):
        n = 1
        # Automatically connect the client to the AP until it reaches the maximum device limit.
        try:
            for client in self.client_lst:
                if len(self.ap_lst[0]) - 13 >= int(self.ap_lst[0][12]):
                    self.client_log.append(f"Step {self.n}: CLIENT ROAM DENIED")
                    self.ap_log.append(f"Step {self.n}: {client[1]} TRIED {self.ap_lst[0][1]} BUT WAS DENIED")
                self.ap_lst[0].append(client[1])
                # Log Roam: Fast roam if the new AP supports 802.11r
                if self.ap_lst[0][10] == 'true':
                    self.client_log.append(f"Step {n}: {client[1]} FAST ROAM TO {self.ap_lst[0][1]}")
                else:
                    self.client_log.append(f"Step {n}: {client[1]} ROAM TO {self.ap_lst[0][1]}")
                self.ap_log.append(f"STEP {n}: {client[1]} CONNECT LOCATION {' '.join(client[2:9])}")
                self.client_log.append("Step " + str(n) + ": " + str(client[1]) + " CONNECT TO " +
                                      self.ap_lst[0][1] + " WITH SIGNAL STRENGTH " +
                                      str(self.calculate_rssi(client[2], client[3], self.ap_lst[0][2],
                                                              self.ap_lst[0][3], self.ap_lst[0][6],
                                                              self.ap_lst[0][5])))
                n += 1
                if client[1] == self.move_lst[0][1]:
                    # If the distance between MOVE and AP is greater than the AP's coverage radius, CLIENT
                    # has gone out of range.
                    if self.calculate_distance(self.move_lst[0][2], self.move_lst[0][3], self.ap_lst[0][2],
                                               self.ap_lst[0][3]) > int(self.ap_lst[0][11]):
                        self.client_log.append(
                            "Step " + str(n) + ": CLIENT DISCONNECT FROM " + str(self.ap_lst[0][1]) +
                            " SIGNAL STRENGTH " + str(self.calculate_rssi(client[2], client[3],
                                                                          self.ap_lst[0][2], self.ap_lst[0][3],
                                                                          self.ap_lst[0][6], self.ap_lst[0][5])))
                        self.ap_log.append(f"Step {n}: {client[1]} DISCONNECTS AT LOCATION "
                                           f"{' '.join(client[2:9])}")
                n += 1
        except IndexError:
            self.ap_log.append(f"Final Step: {self.move_lst[0][1]} HAS NO APS TO CONNECT TO")
            self.client_log.append(f"Final Step: {self.move_lst[0][1]} HAS NO APS TO CONNECT TO")
    ...
    def calculate_rssi(self, coord_1_x, coord_1_y, coord_2_x, coord_2_y, frequency, ap_power):
        ...
        if '/' in str(frequency):
            ...
            max_frequency = float(frequency) * 1000
        return int(ap_power) - (20 * log10(distance)) - (20 * log10(int(max_frequency))) - 32.44
